...
function Client_GameRefresh(game)
	...
	print("Client_GameRefresh")
	Dump(Mod.PlayerGameData)
	print("Updated game")
	RefreshGame(game);
	
    --Check for proposals we haven't alerted the player about yet
    for _,chat in pairs(Mod.PlayerGameData.Chat or {}) do
        DoProposalPrompt(game, chat);
        if (HighestProposalIDSeen < Chat.ID) then
            HighestProposalIDSeen = Chat.ID;
        end
    end
