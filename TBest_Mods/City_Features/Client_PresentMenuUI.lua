...
function Client_PresentMenuUI(rootParent, setMaxSize, setScrollable, game, close)
	...
	if (AlertUnreadChat == nil) then
		AlertUnreadChat = true
	end --Alert the user when they have unread chat
	if (EachGroupButton == nil) then
		EachGroupButton = true
	end --Each group has a button in PresentMenuUi
	if (NumPastChat == nil) then
		NumPastChat = 7
	end --Max amount of past chat shown
	...
	--Manage group button
	UI.CreateButton(horizontalLayout).SetText("Manage groups").SetFlexibleWidth(0.2).SetOnClick(
		function()
	...
	UI.CreateButton(horizontalLayout).SetText("Broadcast").SetFlexibleWidth(0.2).SetColor("#880085").SetOnClick(
		function()
			if (ChatMsgContainerArray ~= {}) then
				DestroyOldUIelements(ChatMsgContainerArray)
			end
	...
	if (PlayerGameData.Chat ~= nil) then
		if not (EachGroupButton) then
			...
			ChatGroupSelectedText =
				UI.CreateButton(horizontalLayout).SetText("Chat Group").SetFlexibleWidth(0.6).SetColor(randomColor())
			--Make a button to select chat group
			UI.CreateButton(horizontalLayout).SetText("Select chat group").SetFlexibleWidth(0.2).SetOnClick(ChatGroupSelected)
		else
			--For all groups, show a button
			for GroupID, group in pairs(PlayerGameData.Chat) do
				UI.CreateButton(horizontalLayout).SetText(PlayerGameData.Chat[GroupID].GroupName).SetColor(
					PlayerGameData.Chat[GroupID].Color
				).SetOnClick(
					function()
	...
	ChatMessageText =
		UI.CreateTextInputField(vert).SetPlaceholderText(" Max 300 characters in one messages").SetFlexibleWidth(0.9).SetCharacterLimit(
		300
	...
	UI.CreateButton(ChatButtonContainer).SetColor("#880085").SetText("Send chat").SetOnClick(
		function()
			if (ChatGroupSelectedID == nil) then
	...
	if (ClientGame.Settings.CommerceGame == true and Mod.Settings.ModGiftGoldEnabled == true) then
		UI.CreateButton(ChatButtonContainer).SetColor("#FFFF00").SetText("Gift gold").SetOnClick(
			function()
...
function SettingsDialog(rootParent, setMaxSize, setScrollable, game, close)
	...
	AlertUnreadChatCheckBox =
		UI.CreateCheckBox(vert).SetIsChecked(AlertUnreadChat).SetText("Show an alert when there is unread chat")
	...
	UI.CreateButton(buttonRow).SetText("Go Back").SetColor("#0000FF").SetOnClick(
		function()
	...
	ResizeChatDialog =
		UI.CreateButton(buttonRow).SetText("Save settings").SetColor("#00ff05").SetOnClick(
		function()
...
function CreateGroupEditDialog(rootParent, setMaxSize, setScrollable, game, close)
	...
	TargetPlayerBtn =
		UI.CreateButton(row1).SetText("Select player...").SetColor(randomColor()).SetOnClick(TargetPlayerClicked)

	row11 = UI.CreateHorizontalLayoutGroup(vert)
	GroupTextNameLabel = UI.CreateLabel(row11).SetText("Name a new chat group: ")
	GroupTextName =
		UI.CreateTextInputField(row11).SetCharacterLimit(25).SetPlaceholderText(" Group Name max 25 characters").SetPreferredWidth(
		200
	).SetFlexibleWidth(1)
	...
	UI.CreateButton(row2).SetText("Remove Player").SetColor("#FF0000").SetOnClick(
		function()
			if (TargetPlayerID == nil) then
				UI.Alert("Please choose a player first")
				return
	...
	LeaveGroupBtn =
		UI.CreateButton(buttonRow).SetText("Leave group").SetInteractable(false).SetColor("#FF0000").SetOnClick(
		function()
			--If GroupTextName.GetInteractable is false, we know that TargetGroupID is set
			if (GroupTextName.GetInteractable() == true) then
				UI.Alert("Please choose a group from the list")
	...
	deleteGroupBtn =
		UI.CreateButton(buttonRow).SetText("Delete group").SetInteractable(false).SetColor("#FF0000").SetOnClick(
		function()
			--If GroupTextName.GetInteractable is false, we know that TargetGroupID is set
			if (GroupTextName.GetInteractable() == true) then
				UI.Alert("Please choose a group from the list")
			...
			UI.PromptFromList(
				"Are you sure you want to delete " .. Mod.PlayerGameData.Chat[TargetGroupID].GroupName .. "?",
...
function RefreshChat()
	...
	if (Mod.PublicGameData.GameFinalized == false) then
		if (ChatGroupSelectedID ~= nil) then
			if (PlayerGameData.Chat[ChatGroupSelectedID].NumChat == nil) then
				...
				ChatArrayIndex = PlayerGameData.Chat[ChatGroupSelectedID].NumChat
			end
		end
	end
	...
	if (ChatGroupSelectedID == nil or ChatArrayIndex == 0) then
		...
		for i = startIndex, NumChat do
			...
			BroadcastChatMsg =
				UI.CreateButton(BroadcastChatLayout).SetPreferredWidth(150).SetPreferredHeight(8).SetColor("#880085")
			...
			ChatMessageTextRecived =
				UI.CreateLabel(BroadcastChatLayout).SetFlexibleWidth(1).SetFlexibleHeight(1).SetText(
	...
	for i = startIndex, ChatArrayIndex do
		...
		if (PlayerGameData.Chat[ChatGroupSelectedID][i].Sender == -1) then
			...
			ChatSenderbtn.SetText(
				ClientGame.Game.Players[PlayerGameData.Chat[ChatGroupSelectedID][i].Sender].DisplayName(nil, false)
...
function IsPotentialTarget(player)
	...
	if (ClientGame.Settings.SinglePlayer) then
		return true
	end --in single player, allow proposing with everyone
