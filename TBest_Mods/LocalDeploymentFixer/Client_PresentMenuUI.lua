function Client_PresentMenuUI(rootParent, setMaxSize, setScrollable, game)
	...
	addOrders =
		UI.CreateButton(row1).SetText("Add last turns deployment and transfers").SetOnClick(AddOrdersHelper).SetColor(
		"#00ff05"
	)
	local row2 = UI.CreateHorizontalLayoutGroup(vert)
	addDeployOnly =
		UI.CreateButton(row2).SetText("Add last turns deployment only").SetOnClick(AddDeployHelper).SetColor("#00ff05")
		...
		if (order.PlayerID == Game.Us.ID) then
			if (order.proxyType == "GameOrderDeploy") then
				...
				if (Game.Us.ID == standing.Territories[order.DeployOn].OwnerPlayerID) then
					...
					if (order.NumArmies == 0) then
						break
					end
					if (maxDeployBonues[bonusID] == 0) then
						break
					end
			...
			if (order.proxyType == "GameOrderAttackTransfer") then
				if (Game.Us.ID == standing.Territories[order.From].OwnerPlayerID) then --from us
					if (Game.Us.ID == standing.Territories[order.To].OwnerPlayerID) then -- to us
						newOrder =
							WL.GameOrderAttackTransfer.Create(Game.Us.ID, order.From, order.To, 3, order.ByPercent, order.NumArmies, false)
						table.insert(orderTabel, newOrder)
					end
				end
			end
...
function AddDeployHelper()
	...
	Game.GetTurn(
		turn,
		function(data)
			getTurnHelperAdd(data)
		end
	)
 --getTurnHelperAdd(data) end)
end
