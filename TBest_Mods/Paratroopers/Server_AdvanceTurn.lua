...
function Server_AdvanceTurn_Order(game, order, result, skipThisOrder, addNewOrder)
	if (order.proxyType == "GameOrderCustom" and startsWith(order.Payload, "Paratroopers_")) then --look for the order that we inserted in Client_PresentMenuUI
		...
		local incomeMod =
			...
			order.PlayerID,
			numArmies * -2,
			numArmies .. " paratroopers attacked " .. game.Map.Territories[targetTerritoryID].Name
		)

		--Check if the player can afford the attack. If we can't afford, skip
		PlayerCurrentIncome =
			game.Game.Players[order.PlayerID].Income(0, game.ServerGame.LatestTurnStanding, false, false).Total
		...
		if (numArmies > armiesOnTerritory) then
			numArmies = armiesOnTerritory
		end
