...
function Server_AdvanceTurn_Order(game, order, result, skipThisOrder, addNewOrder)
			...
			if (isAtWar(game, order) == false) then --not at war? skip the attack
				addNewOrder(
					WL.GameOrderEvent.Create(
						order.PlayerID,
						"You are not at war with the owner of " .. game.Map.Territories[order.To].Name,
						{}
					)
				)
	...
	if (order.proxyType == "GameOrderPlayCardSpy") then
		addNewOrder(
			WL.GameOrderEvent.Create(order.PlayerID, order.PlayerID .. " is now at war with " .. order.TargetPlayerID, nil)
		) --Order is public
	end
	--play a spy card, so we can remeber the event
	if (order.proxyType == "GameOrderCustom") then -- TODO should make sure that the GameOrderCustom is from our mod, if we ever work on this mod again
		print(order.Payload)
